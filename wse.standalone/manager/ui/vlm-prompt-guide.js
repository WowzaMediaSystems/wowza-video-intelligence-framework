/* VLM Custom-mode "Prompting guide" panel (VI-614 follow-up).
 *
 * Its own module (loaded via dynamic <script> injection alongside
 * vlm-schema-builder.js) so this explanatory UI logic stays out of
 * stream-config.html.
 *
 * Renders the standalone VLM default prompts + schema LIVE from
 * `defaultConfig.vlm_defaults` (relayed by the WSE /config endpoint from VIS,
 * GET /vlm/defaults) — never a hardcoded mirror, so it can't drift from what the
 * analyzer actually falls back to. Falls back to static guidance when VIS is
 * unreachable (the field is absent).
 *
 * Depends on globals from stream-config.html: defaultConfig, markDirty,
 * updateVlmCustomLint. Public surface on window: openVlmPromptGuide,
 * closeVlmPromptGuide, useVlmPromptTemplate.
 */
(function () {
    'use strict';
