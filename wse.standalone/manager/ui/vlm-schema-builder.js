/* VLM custom response_schema builder (VI-600 / VI-614).
 *
 * Extracted from stream-config.html so the (recursive) builder logic lives in its
 * own module instead of bloating the page. Loaded via a dynamically-injected
 * <script> (see loadVlmSchemaBuilderModule() in stream-config.html), mirroring the
 * listeners/*.js loading pattern; files in docker/manager/ui/ are served under
 * wse-plugins/server/vif/.
 *
 * Dependency: the global markDirty() defined in stream-config.html (called via the
 * dirty() guard, so the module is harmless if loaded standalone, e.g. in a test
 * harness).
 *
 * The Output Schema editor has two sub-modes that convert losslessly:
 *   Fields  — a recursive per-field builder that GENERATES a JSON Schema, so the
 *             operator never hand-writes JSON for the common case.
 *   Raw JSON — an arbitrary schema, validated as JSON, for anything the builder
 *             can't represent.
 * Field types: string | number | integer | boolean | string[] | enum | object |
 * object[]. enum is a string-valued allowed-list. Anything the builder cannot
 * faithfully rebuild (non-string enums, numeric/string constraints, $ref,
 * oneOf/anyOf, tuple arrays, additionalProperties, …) round-trips through Raw
 * JSON, so a schema is never silently dropped.
 *
 * Public API exposed on window (consumed by stream-config.html + HTML handlers):
 *   collectVlmSchema()        -> schema object | null   (buildConfigJson)
 *   loadVlmSchema(schema)                               (populateForm)
 *   validateVlmSchema()       -> error message | null   (oninput + save guard)
 *   switchVlmSchemaMode(target)                         (Fields/Raw radios)
 *   addVlmSchemaField()                                 (top "+ Add field" button)
 *   resetVlmSchemaBuilder()                             (resetForm)
 */
(function () {
    'use strict';
