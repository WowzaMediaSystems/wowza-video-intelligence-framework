...
**6. Clean up** — the job record when you no longer need it, and the video itself once nothing will analyze it again. Each answers `204` with no body:

```bash
curl -s -u "$WSE_ADMIN_USER:$WSE_ADMIN_PASSWORD" -X DELETE "$VIF/vod/jobs/$JOB_ID"
curl -s -u "$WSE_ADMIN_USER:$WSE_ADMIN_PASSWORD" -X DELETE \
  "$VIF/vod/files?file=vi-object-detection-landscape.mp4"
```
...
- **Codec:** an **H.264** or **H.265** video track for the frame-decoded types (`object`, `scene`, `vlm`). `synthetic` needs **H.264**: it relays the encoded bitstream rather than decoding it, and is refused at once on anything else.
...
A manifest is loaded at startup only if its filename matches its `job_id` and its `manifest_version` is one this Engine reads: a copied, renamed or newer manifest is left on disk, unloaded, with a warning in the VI log naming the file. A download that is in progress when its job is deleted or evicted still completes whole.

The content directory is a separate matter: nothing about a job removes its source file, and the only API that does is [`DELETE /vod/files`](#delete-vodfilesfilerelative-path). While an upload is in flight, a `.upload-<n>.part` temp file sits beside its target; it carries no analyzable extension, so it is never listed or submittable, and it is renamed into place when the upload completes. If the Engine went down mid-upload the temp file stays behind — the Engine sweeps such strays out of the content directory (down to the six levels the listing looks at) once, at its next start.
...
`results` is server-relative — the Engine does not know the host name you reach it by. It is absent from one kind of delivery: a terminal event re-sent at Engine start for a job that same start also forgot (the job cap or TTL retired it before the event had been confirmed) — the ending is still reported, but nothing serves its rows any more. The payload is otherwise the collection view of the job, so it carries no `config` or `effective_config`; read the job for those.
...
The global destination authenticates through the same document: the VOD settings' `lifecycle_webhook_secret` names the entry sent on deliveries to its `lifecycle_webhook` — and only there; a job that overrides the URL names its own secret, and a job-named secret wins over the global one when both would apply. A settings patch whose `lifecycle_webhook_secret` names no configured secret is refused with a `400`, and so is a secrets patch that would remove the very entry the settings reference. A settings patch whose `lifecycle_webhook` is not an absolute http(s) URL is refused the same way; a `${ENV_VAR}` placeholder is accepted and judged when it resolves.
...
| `lifecycle_webhook` | none | Default status-webhook URL for every job; an absolute http(s) URL, or a `${ENV_VAR}` placeholder resolving to one. Anything else is refused. |
...
- **Delete source files over the API once their jobs are terminal.** `DELETE /vod/files?file=…` removes one; a finished job keeps its results without the file, a cancelled or failed one can no longer be resumed, and the delete is refused while a queued or running job is using the file.
- **Usage note:** file analysis is metered by media time analyzed, exactly as reported in the manifest's `media_ms_analyzed`.

## API reference

All endpoints are under the Engine REST API (default `http://localhost:8087`), path prefix `/v2/vif`, authenticated with the Engine's REST admin credentials (see the note in [Quick start](#quick-start)), JSON in and out unless noted. The API's OpenAPI specification is the complete and authoritative reference; when this guide and the specification disagree, the specification is right.
...
| `404` | The job id in the path names no job — and, on the results endpoints, a job that stored no rows; a file delete naming a file that is not there |
| `409` | A conflict with the job's own state: cancelling a terminal job, deleting one that is not terminal, every resume refusal, an upload whose name is taken, a file delete while a queued or running job is using the file |
...
A listing is not a probe: a file here can still turn out to carry no video track this Engine can decode — or an H.265 one, which only the frame-decoded types accept — which the job reports when it runs.
...
### `DELETE /vod/files?file=<relative path>`

Removes one source file from the content root — the file named exactly as the listing names it, and nothing else. Answers `204` with no body.

```bash
curl -s -u "$WSE_ADMIN_USER:$WSE_ADMIN_PASSWORD" -X DELETE "$VIF/vod/files?file=uploads/clip.mp4"
```

What goes is the path as named: a symbolic link is removed as the link, never what it points at. What stays is everything else — no job record, stored results or thumbnail is touched (those are the job's, removed by [`DELETE /vod/jobs/{jobId}`](#delete-vodjobsjobid)), and a subdirectory the delete emptied is left in place.

Files are not owned by jobs, so the delete asks only one thing of them: that no **queued or running** job is reading the file. One that is answers `409` naming the job — cancel it first, then delete:

```json
{"title": "Conflict", "status": 409,
 "detail": "job 1164af1b-… is RUNNING on 'uploads/clip.mp4'; cancel it first (POST /vod/jobs/1164af1b-…/cancel), then DELETE the file"}
```

Terminal jobs neither block the delete nor go with it. A completed job keeps its record and results without its source. A failed or cancelled job keeps its record too, but can no longer be resumed: the resume answers `409` saying the source file cannot be resolved, and automatic resume stands down.

`404` when no such file is there — a second delete of the same name is one, which is the state you asked for. `400` for the names the upload refuses too: no `file`, an absolute or escaping path, or an extension that is not `.mp4`, `.m4v`, `.mov` or `.f4v`. The content directory is the Engine's playback directory, and this endpoint removes only what the listing could show.
...
Stops a queued, connecting, or running job. On a `failed` job that is waiting on an automatic retry (see [resume](#resume)), it drops that retry instead — the job stays `failed`, its record and results intact. No body.
...
Refusals: `404` (unknown job), and `409` on a job that has already finished with no retry pending — there is nothing left to stop:
...
**The job fails immediately with `source_error`.** The file is missing, unreadable, or carries no H.264 or H.265 video track. The extension filter on `/vod/files` is not a probe — a `.mp4` with only AV1 or VP9 inside lists fine and fails here.
...
**A file DELETE answers 409 naming a job.** A queued or running job is reading that file. `POST …/cancel` it, poll until its state is `cancelled`, then DELETE the file again — or let the job finish first.

**A resume answers 409 "the source file for this job cannot be resolved".** The file was deleted or moved after the job ran. Put the same bytes back under the same name and resume again, or submit a new job.

**A cancel answers 409 "only a queued or running job can be cancelled".** The job already reached `completed`, `failed`, or `cancelled` — there is nothing left to stop. DELETE it if what you wanted was the record gone. The one terminal job a cancel accepts is a `failed` one still waiting on its automatic retry: that cancel drops the retry and answers `202`.
