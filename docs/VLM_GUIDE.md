...
| **Medium** | `"medium"` | booleans, preceded by an internal `scene` characterization | fast | you want most of High's grounding at a fraction of the cost — a good default when throughput matters |
| **Low** | `"low"` | one boolean per class | fastest | maximum throughput; the scene is simple and false positives are cheap to tolerate |

Why the middle rung exists: High's free-text `reasoning` (~200 output tokens/request) dominates decode latency, but it also does real grounding work — dropping it entirely (Low) can make a model call *everything* present on ambiguous footage like color-bars test patterns. **Medium** keeps a tiny (~8–20 token) `scene` field that forces the model to characterize the footage *before* it emits the booleans, recovering most of that grounding for a few extra tokens. The `scene` value is an internal deliberation aid only — it is **never** shown on any output surface.

Accepted tradeoff: Low has no safety net beyond its guardrail prompt and `temperature: 0.0`, and Medium adds only the `scene` anchor — either can still return an all-present result on adversarial input. Pick the level to match how costly a false positive is for your use case.
