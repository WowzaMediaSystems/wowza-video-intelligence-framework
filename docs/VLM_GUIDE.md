...
| `reasoning_level` | `"high"` | Detect only: `"high"` (default) / `"medium"` / `"low"` picks how much the model deliberates (see above). Set it from the Manager UI, which also writes the preset `system_prompt`/`user_prompt`/`response_schema` that Low and Medium rely on — the field does not work alone |
