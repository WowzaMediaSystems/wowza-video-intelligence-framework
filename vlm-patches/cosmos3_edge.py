...
# "There is no module or parameter named 'layers'". v0.27.0 fixed this file
# (PR #49190) but its image is broken for Edge anyway: it bundles
# transformers 5.15.0, which removed `get_image_size` from qwen3_vl video
# processing while vLLM's Edge PROCESSOR shim still imports it. Mounting
# v0.27.0's model file over v0.26.0 (transformers 5.14.x) gets the fixed
# code onto a working base. Verified compatible: identical imports, and the
# one extra hook it overrides (_expands_only_video_token) already exists in
# v0.26.0's Qwen3VL base classes. Delete this patch (and its mount) once a
# stable image ships with both fixes.
